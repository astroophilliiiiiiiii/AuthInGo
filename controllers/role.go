...
func NewRoleController(__RoleService services.RoleService) *RoleController {
...
func (rc *RoleController) AssignRoleToUser(w http.ResponseWriter, r *http.Request) {
	...
	utils.WriteJsonSuccessResponse(w, http.StatusOK, "Role assigned to user successfully", "")

}

func (rc *RoleController) GetRoleById(w http.ResponseWriter, r *http.Request) {
	roleId := r.PathValue("id") // Extract role ID from URL parameters --- gives string
	if roleId == "" {
		utils.WriteJsonErrorResponse(w, http.StatusBadRequest, "Role ID is required", fmt.Errorf("missing role ID"))
		return
	}
	...
	role, err := rc.RoleService.GetRoleById(int(id))
	if err != nil {
...
func (rc *RoleController) UpdateRole(w http.ResponseWriter, r *http.Request) {
	roleId := chi.URLParam(r, "id") // "id" parameter nikalo r se
	if roleId == "" {
		utils.WriteJsonErrorResponse(w, http.StatusBadRequest, "Role ID is required", fmt.Errorf("missing role ID"))
		return
	}
