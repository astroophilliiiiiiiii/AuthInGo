...
func (ur *UserRouter) Register(chiR chi.Router) {
	...
	chiR.With(middlewares.JWTAuthMiddleware, middlewares.RequireAllRoles("user")).Get("/profile", ur.UserController.GetUserById) // Context wali ID ke liye
	chiR.With(middlewares.JWTAuthMiddleware).Get("/profile/{id}", ur.UserController.GetUserById)                                 // URL wali ID ke liye

	chiR.With(middlewares.UserCreateRequestValidator).Post("/signup", ur.UserController.Create)
